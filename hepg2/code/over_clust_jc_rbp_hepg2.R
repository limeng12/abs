...
for(g in gene_ids_noctl){
  ...
  t_sj<-read.table(paste0("hepg2/data/star_target_only_jc/",g,"_no_ctl.bed"),sep = "\t",header = FALSE, as.is = TRUE)[,1:7];
...
cat("RBP1\tRBP2\tvalue\tA and B\tA\tB\tA minus B\t B minus A\tnot in AB\n",
    file="hepg2/result/jc_overlap_p_values_melt.tsv");
  ...
  for(g2 in gene_ids_noctl){
    ...
    if(g1>g2){
      ...
      cat(paste0(g1,"\t",g2,"\t",diss_mat[g1,g2],"\t",aa,"\t",a,"\t",b,"\t",ab,"\t",ba,"\t",aabb,"\n"),
          sep="",file="hepg2/result/jc_overlap_p_values_melt.tsv",append = TRUE);
...
pdf("hepg2/result/jc_ovlap_clu.pdf",width = 50,height = 50);
#plot(a);
