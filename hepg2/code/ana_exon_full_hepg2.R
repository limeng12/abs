...
ctl_sj_sj_only<-read.table("hepg2/data/star_mer/CTL_all_tab",sep = "\t",header = TRUE, as.is = TRUE);
...
ctl_sj<-read.table("hepg2/data/exon_mer/CTL_all_tab",sep = "\t",header = TRUE, as.is = TRUE);

ctl_sj_only_polyA_no_shCTL<-read.table("hepg2/data/exon_mer/ENCSR985KAT_CTL_poly___tab",sep = "\t",header = TRUE, as.is = TRUE);
...
for(g in gene_ids){
  ...
  if(!file.exists(paste0("hepg2/data/exon_mer/",g,"_tab"))){
    print(paste0("don't have ",g))
    next;
  }
  
    gene_sj_sj_only<-read.table(paste0("hepg2/data/star_mer/",g,"_tab"),sep = "\t",header = TRUE, as.is = TRUE);

  
    gene_sj_3ss<-str_c(gene_sj_sj_only$chr,":",gene_sj_sj_only$X3_pos);

  
  t_sj<-read.table(paste0("hepg2/data/exon_mer/",g,"_tab"),sep = "\t",header = TRUE, as.is = TRUE);
  ...
  write.table(cbind(t_only_sj_all_bed,1),
              file=paste0("hepg2/data/exon_mer_target_only/",g,"_no_ctl.bed"), sep="\t",
  ...
  write.table(cbind(t_only_sj_all_bed_output,1),
              file=paste0("hepg2/data/exon_mer_target_only_intron/",g,"_no_ctl.bed"), sep="\t",
              quote = FALSE,col.names = FALSE,row.names = FALSE);
...
gene_read_fr_fr<-read.table("hepg2/result/rbp_read_count_map.tsv",sep = "\t",as.is = TRUE,header = TRUE);
