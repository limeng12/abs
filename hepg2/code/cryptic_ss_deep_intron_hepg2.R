...
trans_anno<-read.table("anno/gencode.v29lift37.annotation_transcript.gtf",header = FALSE,as.is = TRUE,sep="\t");

trans_anno<-trans_anno[trans_anno[,3]=="transcript",];
trans_anno<-trans_anno[,c(1,4,5)];
...
#exon_anno<-read.table("anno/hg19_refgene.gtf",header = FALSE,as.is = TRUE,sep="\t");
...
for(g in gene_ids){
  ...
  ss5_in_intron<-countOverlaps(t_sj_gr_5ss,deep_intron_anno,type="within");
  ss3_in_intron<-countOverlaps(t_sj_gr_3ss,deep_intron_anno,type="within");
  
  
  ss5_not_in_exon_count<-sum(ss5_in_intron>0);
  ss3_not_in_exon_count<-sum(ss3_in_intron>0);
  
  
  
  if(ss5_not_in_exon_count>0){
    
    t_5_seqs<-getSeq(hg19_genome,t_sj_5ss[(ss5_in_intron>0)&(t_sj_5ss$strand=="+"),"chr"],
                     t_sj_5ss[(ss5_in_intron>0)&(t_sj_5ss$strand=="+"),"start"]-3,
                     t_sj_5ss[(ss5_in_intron>0)&(t_sj_5ss$strand=="+"),"start"]+5,
                     strand=t_sj_5ss[(ss5_in_intron>0)&(t_sj_5ss$strand=="+"),"strand"],as.character=TRUE);
    #cat(as.character(t_5_seqs),file=paste0("hepg2/data/star_abs5_motif_deep_intron/",g,".seq"), sep="\n");
    
    t_5_seqs_neg<-getSeq(hg19_genome,t_sj_5ss[(ss5_in_intron>0)&(t_sj_5ss$strand=="-"),"chr"],
                         t_sj_5ss[(ss5_in_intron>0)&(t_sj_5ss$strand=="-"),"start"]-5,
                         t_sj_5ss[(ss5_in_intron>0)&(t_sj_5ss$strand=="-"),"start"]+3,
                         strand=t_sj_5ss[(ss5_in_intron>0)&(t_sj_5ss$strand=="-"),"strand"],as.character=TRUE);
    #cat(as.character(t_5_seqs_neg),file=paste0("hepg2/data/star_abs5_motif_deep_intron/",g,".seq"), sep="\n",append = TRUE);
    t_5_seqs<-c(t_5_seqs,t_5_seqs_neg)
    
    weblogo(t_5_seqs,file.out=paste0("hepg2/data/star_abs5_motif_deep_intron/",g,".pdf") ,format="pdf" ,open = FALSE )
    
  }
  
  
  if(ss3_not_in_exon_count>0){
    t_3_seqs<-getSeq(hg19_genome,t_sj_3ss[(ss3_in_intron>0)&(t_sj_3ss$strand=="+"),"chr"],
                     t_sj_3ss[(ss3_in_intron>0)&(t_sj_3ss$strand=="+"),"start"]-19,
                     t_sj_3ss[(ss3_in_intron>0)&(t_sj_3ss$strand=="+"),"start"]+3,
                     strand=t_sj_3ss[(ss3_in_intron>0)&(t_sj_3ss$strand=="+"),"strand"],as.character=TRUE);
    #cat(as.character(t_3_seqs),file=paste0("hepg2/data/star_abs3_motif_deep_intron/",g,".seq"), sep="\n");
    
    t_3_seqs_neg<-getSeq(hg19_genome,t_sj_3ss[(ss3_in_intron>0)&(t_sj_3ss$strand=="-"),"chr"],
                         t_sj_3ss[(ss3_in_intron>0)&(t_sj_3ss$strand=="-"),"start"]-3,
                         t_sj_3ss[(ss3_in_intron>0)&(t_sj_3ss$strand=="-"),"start"]+19,
                         strand=t_sj_3ss[(ss3_in_intron>0)&(t_sj_3ss$strand=="-"),"strand"],as.character=TRUE);
  ...
  g_53_sj<-rbind(g_53_sj,c(g,ss5_not_in_exon_count,ss3_not_in_exon_count,
                           ss5_not_in_exon_count_per,ss3_not_in_exon_count_per) );
  
}


g_53_sj_data<-as.data.frame(g_53_sj,stringsAsFactors=FALSE);
#colnames(g_53_sj_data)<-c("rbp","cryptic_5ss_not_near_exon","cryptic_3ss_not_near_exon");
colnames(g_53_sj_data)<-c("rbp","cryptic_5ss_deep_intron","cryptic_3ss_deep_intron",
                          "cryptic_5ss_deep_intron/total 5ss","cryptic_3ss_deep_intron/total 3ss");

#g_53_sj_data[,"cryptic_5ss_over_3ss_not_near_exon"]<-g_53_sj_data[,"cryptic_5ss_not_near_exon"]/g_53_sj_data[,"cryptic_3ss_not_near_exon"]


write.table(g_53_sj_data,file="hepg2/result/g_53_deep_intron_full.tsv",sep="\t",
...
g_53_sj_data_read[,"normalized cryptic_5ss_deep_intron"]<-as.numeric(g_53_sj_data_read[,"cryptic_5ss_deep_intron"])/
  as.numeric(g_53_sj_data_read$read_count)*(10^6);

g_53_sj_data_read[,"normalized cryptic_3ss_deep_intron"]<-as.numeric(g_53_sj_data_read[,"cryptic_3ss_deep_intron"])/
  as.numeric(g_53_sj_data_read$read_count)*(10^6);

write.table(g_53_sj_data_read,file="hepg2/result/g_53_deep_intron_full.tsv",sep="\t",
            quote = FALSE,col.names = TRUE,row.names = FALSE);
