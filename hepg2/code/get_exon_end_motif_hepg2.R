...
gene_ids<-c(unique(readLines("samples/gene_id_hepg2.txt"))  );
...
for(g in gene_ids){
  ...
  if(!file.exists(paste0("hepg2/data/exon_mer_target_only_small_intron/",g,"_no_ctl.bed"))){
    print(paste0("don't have ",g))
    next;
  }
  
  t_sj<-read.table(paste0("hepg2/data/exon_mer_target_only_small_intron/",g,"_no_ctl.bed"),sep = "\t",header = TRUE, as.is = TRUE);
  ...
  weblogo(intron_seq_all,file.out=paste0("hepg2/data/exon_mer_target_only_small_intron_motif/",g,".pdf") ,open = FALSE )
