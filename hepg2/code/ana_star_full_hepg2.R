...
ctl_sj_high_confi<-ctl_sj_high[ctl_sj_high$anno>2,];
...
for(g in gene_ids){
  ...
  if(!file.exists(paste0("hepg2/data/star_mer/",g,"_tab")) ){
    print(paste0("don't have: ",g) );
    next;
  }
  
  t_sj<-read.table(paste0("hepg2/data/star_mer/",g,"_tab"),sep = "\t",header = TRUE, as.is = TRUE);
  
  colnames(t_sj)<-c("chr","X5_pos","X3_pos","strand","type","X5_n","X3_n","anno");
  
  t_sj<-t_sj[str_detect(t_sj$chr,"chr"),];
  #t_sj<-t_sj[(t_sj$anno>1) & (t_sj$type>0),];
  ...
  if(nrow(t_sj_in_only_5ss_fr)>0){
    ...
    write.table(cbind(t_sj_in_only_5ss_fr,1),file=paste0("hepg2/data/star_target_only_jc_5ss/",g,"_no_ctl.bed"), sep="\t",
  ...
  if(sum(!t_5_only_sj_set_index)>0){
     ...
     cat(as.character(t_5_seqs),file=paste0("hepg2/data/star_abs5_motif/",g,".seq"), sep="\n");
     
     t_5_seqs_neg<-getSeq(hg19_genome,t_sj[(!t_5_only_sj_set_index)&(!positive_index),"chr"],
                      t_sj[(!t_5_only_sj_set_index)&(!positive_index),"X5_pos"]-5,
                      t_sj[(!t_5_only_sj_set_index)&(!positive_index),"X5_pos"]+3,
                      strand=t_sj[(!t_5_only_sj_set_index)&(!positive_index),"strand2"],as.character=TRUE);
     cat(as.character(t_5_seqs_neg),file=paste0("hepg2/data/star_abs5_motif/",g,".seq"), sep="\n",append=TRUE);
  ...
  if(sum(!t_3_only_sj_set_index)>0){
    t_3_seqs<-getSeq(hg19_genome,t_sj[(!t_3_only_sj_set_index)&positive_index,"chr"],
     ...
     cat(as.character(t_3_seqs),file=paste0("hepg2/data/star_abs3_motif/",g,".seq"), sep="\n");
    
    t_3_seqs_neg<-getSeq(hg19_genome,t_sj[(!t_3_only_sj_set_index)&(!positive_index),"chr"],
                     t_sj[(!t_3_only_sj_set_index)&(!positive_index),"X3_pos"]-3,
                     t_sj[(!t_3_only_sj_set_index)&(!positive_index),"X3_pos"]+19,
                     strand=t_sj[(!t_3_only_sj_set_index)&(!positive_index),"strand2"],as.character=TRUE);
    cat(as.character(t_3_seqs_neg),file=paste0("hepg2/data/star_abs3_motif/",g,".seq"), sep="\n",append = TRUE);
    
    t_3_seqs<-c(t_3_seqs,t_3_seqs_neg);
    
    weblogo(as.character(t_3_seqs),file.out=paste0("hepg2/data/star_abs3_motif/",g,".pdf"),format="pdf" ,open = FALSE );
  ...
  if(nrow(t_sj_in_only_3ss_fr)>0){
    ...
    write.table(cbind(t_sj_in_only_3ss_fr,1),file=paste0("hepg2/data/star_target_only_jc_3ss/",g,"_no_ctl.bed"), sep="\t",
              quote = FALSE,col.names = FALSE,row.names = FALSE);
  }
  ...
  if("chrM" %in% t_only_sj_all$chr){
    write.table(t_only_sj_all[t_only_sj_all$chr=="chrM",],
                file=paste0("hepg2/data/star_wrong_jc/",g,"_no_ctl_somethingwrong.bed"), sep="\t",
                quote = FALSE,col.names = FALSE,row.names = FALSE);
  }
...
write.table(g_53_sj_data,file="hepg2/result/g_53_sj_all_full_hepg2.tsv",sep="\t",
...
gene_read_fr_fr<-read.table("hepg2/result/rbp_read_count_map.tsv",header = TRUE,as.is = TRUE,sep="\t")
...
write.table(g_53_sj_data_read,file="hepg2/result/g_53_sj_all_read_count_hepg2.tsv",sep="\t",
            quote = FALSE,col.names = TRUE,row.names = FALSE);
