...
intron_coor<-intron_coor[!duplicated(intron_coor),];
...
GT_pos<-str_locate_all(seq_list_all_v,"GT");

intron_start<-start(intron_coor_gr);

intron_end<-end(intron_coor_gr);

intron_seqnames<-seqnames(intron_coor_gr);

all_gt_seq<-c();
for(i in 1:length(GT_pos) ){
  one_intron_gt<-GT_pos[[i]];
  
  if(nrow(one_intron_gt)<2){
    next;
  }
  
  for(j in 1:(nrow(one_intron_gt)-1) ){
    if(length(all_gt_seq)>10000){
      break;
    }
    
    #print(paste0("start pos: ",intron_start[i]+one_intron_gt[j,"start"]," chr: ",intron_seqnames[i]) );
    
    aaaa<-as.character(  getSeq(genome,intron_seqnames[i],intron_start[i]+one_intron_gt[j,"start"]-3,
                                intron_start[i]+one_intron_gt[j,"start"]+5) );
    
    all_gt_seq<-c(all_gt_seq,aaaa);
...
weblogo(all_gt_seq,file.out=paste0("result/anno_all_cryptic_gt_score.pdf") ,open = FALSE )


writeLines(all_gt_seq,con = file("result/anno_all_cryptic_gt_score.seq") );

setwd("/Users/mengli/Documents/software/maxentscan");

system(" perl score5.pl /Users/mengli/Documents/projects/abs/result/anno_all_cryptic_gt_score.seq > /Users/mengli/Documents/projects/abs/result/anno_all_cryptic_gt_score.score");
