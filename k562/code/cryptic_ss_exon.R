...
for(g in gene_ids){
  ...
  if(ss5_in_exon_count>0){
    ...
    #cat(as.character(t_5_seqs),file=paste0("data/star_abs5_motif_exon/",g,".seq"), sep="\n");
    
    t_5_seqs_neg<-getSeq(hg19_genome,t_sj_5ss[(ss5_in_exon>0)&(t_sj_5ss$strand=="-"),"chr"],
                         t_sj_5ss[(ss5_in_exon>0)&(t_sj_5ss$strand=="-"),"start"]-5,
                         t_sj_5ss[(ss5_in_exon>0)&(t_sj_5ss$strand=="-"),"start"]+3,
                         strand=t_sj_5ss[(ss5_in_exon>0)&(t_sj_5ss$strand=="-"),"strand"],as.character=TRUE);
    #cat(as.character(t_5_seqs_neg),file=paste0("data/star_abs5_motif_exon/",g,".seq"), sep="\n",append = TRUE);
    t_5_seqs<-c(t_5_seqs,t_5_seqs_neg)
    
    weblogo(t_5_seqs,file.out=paste0("data/star_abs5_motif_exon/",g,".pdf") ,format="pdf" ,open = FALSE )
  ...
  if(ss3_in_exon_count>0){
    ...
    #cat(as.character(t_3_seqs),file=paste0("data/star_abs3_motif_exon/",g,".seq"), sep="\n");
    
    t_3_seqs_neg<-getSeq(hg19_genome,t_sj_3ss[(ss3_in_exon>0)&(t_sj_3ss$strand=="-"),"chr"],
                         t_sj_3ss[(ss3_in_exon>0)&(t_sj_3ss$strand=="-"),"start"]-3,
                         t_sj_3ss[(ss3_in_exon>0)&(t_sj_3ss$strand=="-"),"start"]+19,
                         strand=t_sj_3ss[(ss3_in_exon>0)&(t_sj_3ss$strand=="-"),"strand"],as.character=TRUE);
    #cat(as.character(t_3_seqs_neg),file=paste0("data/star_abs3_motif_exon/",g,".seq"), sep="\n",append = TRUE);
    
    
    t_3_seqs<-c(t_3_seqs,t_3_seqs_neg)
    weblogo(t_3_seqs,file.out=paste0("data/star_abs3_motif_exon/",g,".pdf") ,format="pdf" ,open = FALSE )
...
write.table(g_53_sj_data,file="result/g_53_in_exon_full.tsv",sep="\t",
...
write.table(g_53_sj_data_read,file="result/g_53_in_exon_full.tsv",sep="\t",
            quote = FALSE,col.names = TRUE,row.names = FALSE);
